...
class SweetieFixes:
    ...
    warn = "(WARNING! .urdf file will be overwited from .orig.urdf)"
    ...
    def is_valid_pkg(self, arg):
        if not arg in rospack.list():
            ...
            if self.is_valid_file(rospack.get_path(arg) + '/urdf/' + arg + '.urdf') or \
                    self.is_valid_file(rospack.get_path(arg) + '/urdf/orig_' + arg + '.urdf'):
                return arg

    def get_urdf_file_name(self, arg):
        if not arg in rospack.list():
            self.parser.error("The package %s does not exist!!" % arg)
        else:
            path = rospack.get_path(arg) + '/urdf/'
            orig_urdf = path + 'orig_' + arg + '.urdf'
            result_urdf = path + arg + '.urdf'
            if not os.path.isfile(orig_urdf):
                if os.path.isfile(result_urdf):
                    copyfile(result_urdf, orig_urdf)
                else:
                    self.parser.error("The file %s does not exist!" % result_urdf)
            else:
                copyfile(orig_urdf, result_urdf)
            return result_urdf

    def read_cmdline_args(self):
        self.parser.add_argument('-t', '--tui', action='store_true', default=True, help='Show ncurses tui')
        self.parser.add_argument("-f", "--file", required=False, type=self.is_valid_file, metavar="FILE",
                                 help="the URDF file to apply fixes")
        self.parser.add_argument("-p", "--pkg", required=False, type=self.is_valid_pkg, metavar="PKG",
                                 help="robot description package name " + self.warn)
        ...
        self.file = self.pkg if self.pkg is not None else self.file
        if self.file is None:
            if self.rollout is not None or self.rollback is not None:
            ...
            if self.rollout is None and self.rollback is None:
                ...
                self.plugin_choices_list = self.rollout if self.rollback is None else self.rollout
                self.file = self.get_urdf_file_name(self.pkg)
                self.tui = False
                return True
    ...
    def read_tui(self):
        description_pkgs = [(pkg, '') for pkg in
                            filter(lambda x: re.search('sweetie_bot_[\w]+?_description', x), rospack.list())]
        ...
        code, self.pkg = d.menu(self.description + self.what_file + self.warn, choices=description_pkgs)
        ...
        for plugin in plugins.loaded.values():
            if 'only_for' in dir(plugin) and self.pkg not in plugin.only_for:
                continue
            if func_name in dir(plugin):
                choices.append((plugin.display_name, plugin.cmd_letter, plugin.enabled))

        code, tags = d.checklist(self.what_func % func_name, choices=choices,
                                 title=func_name + ' ' + os.path.basename(self.file), width=60)
        ...
        for plugin in plugins.loaded.values():
            if (plugin.display_name in tags):
                self.plugin_choices_list.append(plugin.name)

        setattr(self, func_name, self.plugin_choices_list)
        return True
    ...
    def write_robot_to_file(self, filename=None):
        file_string = self.robot.to_xml_string()
        file_string = file_string.replace(self.xml_tag, self.xml_tag + "\n" + self.do_not_edit_warn % ', '.join(
            self.plugin_choices_list))
    ...
    def do(self):
        ...
        for plugin in plugins.loaded.values():
            if plugin.name in getattr(self, func_name):
                ...
                if func(self.robot, self.file, self.pkg):
